...
>    - **PostgreSQL Enum 值固定大寫**（Kit 通用坑 #2 同此原則，但本專案常被「表單送小寫」誤觸，列為坑 #2 持續警戒）
>    - **logo 仍使用 `app/static/images/sela-logo.jpg|svg`**（被 4 個現有模板引用） — V1.0.0 順便加入 Kit 完整 favicon 套組到 `app/static/favicon/`，但 base.html 尚未串接，列為下版候選
...
- **版本：** V1.0.0（首次對齊 Kit V1.9.0 規範）
- **狀態：** 上線中（30 人團隊每日使用）
- **線上網址：** https://online-drink-production.up.railway.app
- **一句話定位：** LINE Login 認證的團體飲料／餐點／團購訂餐系統，給彰濱秀傳特定團隊每日揪團用。
- **技術棧：** Python 3.11 + FastAPI + SQLAlchemy 2.0 + PostgreSQL + Jinja2 + Tailwind（CDN）+ Alpine.js + HTMX
...
| 版本 | 重點 |
|------|------|
| V1.0.0 | **首次對齊 SELA-Starter-Kit V1.9.0**。新增根目錄 `CLAUDE.md` / `README.md` / `SELA-handoff.md`；換用 Kit 標準 `.gitignore`；加入完整 favicon 套組到 `app/static/favicon/`；移除 commit 進去的 `.DS_Store` 與重複的 `gitignore`（無點）檔。**零程式碼變更** — 純文件 / 資產對齊。 |
...
1. **base.html 串接 Kit favicon 套組** — 上版只把資產放進 `app/static/favicon/`，但 `base.html` 還沒加 `<link rel="icon">`。串接後瀏覽器分頁會顯示 SELA logo。**為什麼是第 1 名：** 改動極小（1 段 HTML），純改善 UX，且能完成 V1.0.0 沒收尾的事
2. 訂單匯出 Excel — 原本就在 backlog（見 docs/SELA-開發指導手冊.md「待開發」）
3. 外送費分攤功能 — 原本就在 backlog；`scripts/DELIVERY_FEE_CHANGES.py` 已有設計稿
4. 多尺寸定價 — 之前因 bug 回滾過，重做時注意 schema 三方對齊（坑相關）
5. 菜單匯入開放 `group_buy` category（解坑 #9）
6. 評估是否要把 `taipei` filter 抽到共用 templates 模組（解坑 #6，但要評估重構成本）

---

## 八、升版必讀

V1.0.0 屬「純文件對齊、零程式碼變更」 → 不開警告章節。

### V1.0.0 部署動作

- [ ] 用 Git Pusher 匯入 `線上訂餐 V1.0.0.zip` 上 GitHub
- [ ] **不需要動 Railway Variables**（沒改任何環境變數）
- [ ] **不需要動第三方 Console**（LINE / Cloudinary callback 都沒變）
- [ ] **不需要跑 migration**（沒改 schema）
- [ ] 等待 Railway 自動 redeploy（**約 30-60 秒 cold start**）
- [ ] 部署後測：訪問 `/` 確認首頁正常、登入流程可走完

---

## 九、一句話總結

V1.0.0 完成了首次對齊 SELA-Starter-Kit V1.9.0 — 補上根目錄三大文件（CLAUDE.md / README.md / SELA-handoff.md）+ Kit 標準 .gitignore + 完整 favicon 資產，但 base.html 還沒串接 favicon。**下版第一優先：在 base.html `<head>` 加 favicon `<link>` 標籤，完成 V1.0.0 沒收尾的最後一步。**
